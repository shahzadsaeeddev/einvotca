...
from ..models import JournalLine, JournalProductDetail
...
class JournalLineMasterSerializer(QrCodeMixin, InvoiceSetupMixin, serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            master = JournalLine.objects.create(**validated_data, transaction_type='SALE',
            ...
            for value in items_transactions:
                ...
                JournalProductDetail.objects.create(invoice=master, total_inclusive_tax_amount=tax_inclusive_amount,
                                                    tax_amount=tax_amount, total=total, cost=cost_price,
...
class CreditNoteSerializer(QrCodeMixin, InvoiceSetupMixin, serializers.ModelSerializer):
    items_transactions = JournalProductDetailSerializer(many=True)
    cashier = serializers.CharField(source='created_by.first_name', read_only=True)
    qrcode = serializers.SerializerMethodField()
    ...
    class Meta:
        model = JournalLine
        exclude = ['location', 'deleted', 'ref_no', 'slug', 'updated_at', 'description', 'id']
        extra_kwargs = {
            'party': {'write_only': True},
            'payment_method': {'write_only': True},
            'created_by': {'write_only': True},
    ...
    def create(self, validated_data):
        ...
        journal_line = JournalLine.objects.filter(location=validated_data['location'],
                                                  invoice_no=validated_data['invoice_no'],
                                                  transaction_type="CREDIT_NOTE").first()
        ...
        with transaction.atomic():
            ...
            master = JournalLine.objects.create(**validated_data, transaction_type='CREDIT_NOTE',
                                                transaction_status='PENDING', )
            ...
            for value in items_transactions:
                ...
                item_discount = value.get('discount', 0)
                item_rate = value.get('rate', 0)
                cost = item.cost_price_slot * quantity
                item_name = item.name
                ...
                tax_inclusive_amount = taxable_amount + tax_amount
                JournalProductDetail.objects.create(invoice=master, total_inclusive_tax_amount=tax_inclusive_amount,
                                                    tax_amount=tax_amount, total=total, item_name=item_name, cost=cost,
